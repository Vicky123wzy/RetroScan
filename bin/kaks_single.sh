...
if [[ -s $tmp_path/temp_blast.out ]];then 
...
elif [[ ! -s $tmp_path/temp_blast.out ]];then 
	echo -e $mRNA"&"$retro"\t"$kaksmethod"\tNA\tNA\tNA\t0\t0\t0\t0\tNA\t0\t0\t0\tNA\tNA\t0\t1:1:1:1:1:1\t0\tNA\tNA\tNA\tNA" > $tmp_path/kaks_result.txt
	retro_pro=">"$retro"_frame=1"
	awk '/^>/&&NR>1{print "";}{ printf "%s",/^>/ ? $0"\t":$0 }' $tmp_path/temp_retro_pro.fasta | awk '{if($1=="'"$retro_pro"'"){print $1"\n"$NF}}' > $tmp_path/retro_pro.fasta
	sed -i '1c >'$retro'' $tmp_path/retro_pro.fasta	
	echo -e $retro"\t+\tpseudogene" > $tmp_path/des_retro.txt
fi
