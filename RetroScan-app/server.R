...
shinyServer(function(input, output){
  ...
  final <- reactive({
    final_file <- input$file1
    if(is.null(final_file))
      return(NULL)
    final <- read.table(final_file$datapath,header = T, sep=input$sep,quote = input$quote,stringsAsFactors = F)
    final})
  
  info <- reactive({
    info_file <- input$file2
    if(is.null(info_file))
      return(NULL)
    info <- read.table(info_file$datapath,header = T, sep=input$sep,quote = input$quote,stringsAsFactors = F)
    info})
  
  cds_fasta <- reactive({
    cds_file <- input$cds_file
    if(is.null(cds_file))
      return(NULL)
    cds_fasta <- readDNAStringSet(cds_file$datapath, format="fasta",nrec=-1L, skip=0L, seek.first.rec=FALSE, use.names=TRUE)
    names(cds_fasta) <- gsub(" .*.*","",names(cds_fasta))
    cds_fasta})
  
  fpkm_value <- reactive({
    fpkm_file <- input$fpkm_file
    if(is.null(fpkm_file))
      return(NULL)
    fpkm_file <- read.table(fpkm_file$datapath,header = T, sep=input$sep,quote = input$quote,stringsAsFactors = F)
    fpkm_file})
  ...
  phenotype <- reactive({
    ...
    if(is.null(phenotype_file))
      return(NULL)
    phenotype_file <- read.table(phenotype_file$datapath,header = F, sep=input$sep,quote = input$quote,stringsAsFactors = F)
    phenotype_file})
  ...
  output$Retrocopy_result<-downloadHandler(
    ...
    content=function(file){write.table(final(), file, row.names = F, sep="\t", quote=F)})
  ...
  num_plot <- reactive({
    parent_num <- as.data.frame(table(as.data.frame(table(final()$Parental_gene_ID))$Freq))
    ggplot(parent_num,aes(x=Var1,y=Freq)) + 
      geom_bar(stat="identity",fill=input$num_color) +
      labs(title="The number distribution of retrocopies owned by each parent gene",x="Number of retrocopies",y="Number of parental genes") + 
      theme_classic() + 
  ...
  des_plot <- reactive({
    table_pie <- as.data.frame(table(final()$Description))
  ...
  output$retro_plot <- renderPlot({
    retrocopy_structure(final(),info(),input$sample1,input$utr_color,input$exon_color,input$intron_color,
  ...
  output$retroplotdown <- downloadHandler(
    ...
    content = function(file){
      ...
      print(retrocopy_structure(final(),info(),input$sample1,input$utr_color,input$exon_color,input$intron_color,
                                input$frame_color,input$text_size,input$title_size))
      dev.off()})
  
  output$retroseq <- reactive({
    str = paste(">",input$sample1,sep="")
    sequence <- as.character(final()[final()$Retrocopy_ID %in% input$sample1,"Retro_sequence"])
  ...
  output$retropep <- reactive({
      str = paste(">",input$sample1,sep="")
      sequence <- as.character(final()[final()$Retrocopy_ID %in% input$sample1,"Retro_protein"])
      for(i in 1:ceiling(nchar(sequence)/60)){
          str <- paste(str, substr(sequence,(i-1)*60+1,i*60),sep = '\n')
      }
      str
  ...
  output$alignment111 <- renderPlot({
    final <- final()
    retro_seq <- final[final$Retrocopy_ID %in% input$sample1,"Retro_sequence"]
    pep_id <- final[final$Retrocopy_ID %in% input$sample1,"Parental_gene_ID"]
    pep_seq <- cds_fasta()[names(cds_fasta()) %in% pep_id,]
  ...
  output$kaks_result<-DT::renderDataTable({
    datatable(final()[,c("Retrocopy_ID","Ka","Ks","Ka.Ks")],rownames = FALSE,filter = 'top',
  ...
  output$kaks_result_download<-downloadHandler(
    ...
    content=function(file){write.table(final()[,c("Retrocopy_ID","Ka","Ks","Ka.Ks")], file, row.names = F, sep="\t", quote=F)})
  
  ks_plot <- reactive({
    final <- final()[,c("Retrocopy_ID","Ks")]
  ...
  kaks_plot <- reactive({
    final <- final()[,c("Retrocopy_ID","Ka.Ks")]
  ...
  ks_type <- reactive({
    final <- final()[,c("Retrocopy_ID","Ks","Description")]
  ...
  kaks_type <- reactive({
    final <- final()[,c("Retrocopy_ID","Ka.Ks","Description")]
